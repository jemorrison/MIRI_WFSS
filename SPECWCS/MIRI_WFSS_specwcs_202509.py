"""
This module contains functions to create MIRI WFSS reference files from the code for NIRISS WFSS

Prism
-----

May 29, 2025 AOP
replace NIRISS with MIRI
remove the dx dependence
remove the FWCPOS_REF as that's not a MIRI keyword


July 2023 (R.P.)
----------------
Updates to the SPECWCS file creation function(s):
1) The pedigree and useafter can now be specified.
2) A user can provide a previous SPECWCS file to update rather than creating a
   new file from scratch. This will keep older history in tact while still
   updating the data and adding a new history section. The "history" in the
   meta parameter, however, will only be the latest "history" entry.
3) "convert_conf_to_asdf" was created for a cleaner workflow (this could be done
   independently of creating a new reference file)
...
from jwst.datamodels import MiriWFSSSpecwcsModel
from jwst.datamodels import wcs_ref_models


#-------------------------------------------------------------------------------

def common_reference_file_keywords(author="STScI",
                                   description="MIRI WFSS Reference File",
                                   exp_type=None,
                                   filename="",
                                   filtername=None,
...
def convert_conf_to_asdf(conffile):
    '''
    The *.conf files procuded by create_miri_wfss_conf.py and convert them into
    values we can use for the asdf reference file.

    Parameters
    ----------
    conffile : str
        The text file with configuration information.
        The MIRI WFSS CONF file, aXe format
    ...
    print('orders',orders) 
    for order in orders:
        l0 = beamdict[order]['DISPL'][0] # microns, wavelength for the start of the trace
        l1 = beamdict[order]['DISPL'][1] # microns, length of the trace
        ...
        for disp, xy in zip([dispx, dispy],
                            ['DISPX', 'DISPY']):
            es = beamdict[order][xy]
            disp_order = []
            for e in es:
                cpoly = Polynomial2D(2, c0_0=e[0], c1_0=e[1], c2_0=e[3],
                                     c0_1=e[2], c1_1=e[4], c0_2=e[5])
                disp_order.append(cpoly)
            disp_order = tuple(disp_order)
            disp.append(disp_order)
    ...
    return dispx, dispy, displ, invdispl, orders

#-------------------------------------------------------------------------------


def create_miri_wfss_config(conffile,
                        filter,
                        author="STScI",
                        history=None,
                        outname="test.asdf",
    ...
    configuration, no sensitivity information is included

    Note: The orders are named alphabetically, i.e. Order A, Order B
    There are also sensitivity fits files which are tables of wavelength,
    sensitivity, and error. These are specified in the conffile but will
    ...
    filter : str
        Default = P750L the conffile corresponds to.
    author : str
    ...
    Returns
    -------
    fasdf : asdf.AsdfFile(jwst.datamodels.MiriWFSSSpecmcsModel)
    """

    if not history:
        history = "Created from {0:s}".format(conffile)

    ## open an older specwcs file or create a model of one to fill in

    ref = MiriWFSSSpecwcsModel()

    # Make a dictionary with common reference file keywords for every ref. file
    ref_kw = common_reference_file_keywords(author=author,
                                            description="dispersion model parameters MIRI WFSS",
                                            exp_type="MIR_WFSS",
                                            filename=outname,
                                            filtername=filter,
                                            model_type='MiriWFSSSpecwcsModel',
                                            pedigree=pedigree,
                                            reftype="specwcs",
                                            useafter=useafter,
                                            )

    # convert the conf files into the list format needed for the ref. file
    dispx, dispy, displ, invdispl, orders = convert_conf_to_asdf(conffile)

    ## update the reference file with all the new info
    # 'meta' field:
    ...
    sdict = Software({'name': 'MIRI_WFSS_specwcs_reffiles.py',
                      'author': author,
                      'homepage': 'https://grit.stsci.edu/NIRISS/reference-file-creation/specwcs',
                      'version': '0.1.0'})
...
def create_miri_wfss_wavelengthrange(outname="miri_wfss_wavelengthrange.asdf",
                                 history="MIRI WFSS  wavelengthrange",
                                 author="STScI",
                                 module="N/A",
                                 wavelengthrange=None,
                                 extract_orders=None):
    """Create a wavelengthrange reference file for MIRI WFSS.
    ...
    ref_kw = common_reference_file_keywords(reftype="wavelengthrange",
                                            title="MIRI WFSS wavelengthrange",
                                            exp_type="MIR_WFSS",
                                            description="MIRI WFSS Filter Wavelength Ranges",
                                            useafter="2025-01-01T00:00:00",
                                            author=author,
    ...
    history = HistoryEntry({'description': history,
                            'time': datetime.datetime.utcnow()})
    software = Software({'name': 'MIRI_WFSS_specwcs_reffiles.py',
                         'author': author,
                         'homepage': 'https://grit.stsci.edu/NIRISS/reference-file-creation/specwcs',
                         'version': '0.1.0'})
...
if __name__ == "__main__":
    ...
    conffile = 'MIRI_WFSS_29May2025_1027.conf'
    conffile = 'MIRI_WFSS_03Jun2025_1330.conf'
    conffile = 'MIRI_WFSS_03Jun2025_1433.conf'
    conffile = 'MIRI_WFSS_02Sep2025_1558.conf'
    filter = 'P750L'

                                        
    create_miri_wfss_config(conffile,filter, outname="MIRI_WFSS_specwcs_20250905.asdf")
    create_miri_wfss_wavelengthrange()
